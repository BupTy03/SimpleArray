...
struct SimpleArrayPredicateAdapter {
    ...
    SimpleArrayPredicateAdapter(F f)
        : func { f }
    {
    }
...
public:
    ...
    SimpleArray() {}
    explicit SimpleArray(const size_t sz)
        : sz_ { sz }
    {
    ...
    explicit SimpleArray(const size_t sz, const T& val)
        : sz_ { sz }
    {
        elem_ = reinterpret_cast<T**>(malloc((sizeof(T*) + sizeof(T)) * sz_));
        T* ptr = reinterpret_cast<T*>(elem_ + sz_);
...
public:
    ...
    SimpleArrayIteraror(const SimpleArrayIteraror& it) noexcept
        : p { it.p }
    {
    }
    ...
    inline SimpleArrayIteraror operator+=(difference_type val) noexcept
    {
        p += val;
        return *this;
    }
...
private:
    ConstSimpleArrayIteraror(T** p) noexcept
        : p { p }
    {
    }
...
public:
    ...
    ConstSimpleArrayIteraror(const ConstSimpleArrayIteraror& it) noexcept
        : p { it.p }
    {
    }
    ConstSimpleArrayIteraror(const SimpleArray<T>::SimpleArrayIteraror& it) noexcept
        : p { it.p }
    {
    }
    ...
    inline ConstSimpleArrayIteraror operator+=(difference_type val) noexcept
    {
        p += val;
        return *this;
    }
